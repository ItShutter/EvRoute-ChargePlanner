        ...
        if (totalDistanceKm > currentBatteryKm) {
            ...
            while (currentPosKm + availableKm < totalDistanceKm) {
                ...
                for (const offset of scanOffsets) {
                    ...
                    try {
                        // 💡 ยิงไปหา Backend เส้นทางใหม่ของ Google
                        const url = `/api/stations/google?lat=${scanPoint.lat}&lng=${scanPoint.lng}&distance=30`;
                        const res = await fetch(url);
                        const data = await res.json();

                        if (data && data.length > 0) {
                            stationFound = {
                                data: data[0], // เอาสถานีที่ใกล้ที่สุด (อันดับ 1) ที่ Google หาเจอ
                ...
                if (stationFound) {
                    ...
                    // 💡 แกะโครงสร้างข้อมูลตามรูปแบบของ Google Places API
                    const stLat = st.geometry.location.lat;
                    const stLng = st.geometry.location.lng;
                    const stTitle = st.name || 'สถานีชาร์จ EV';
                    const stOperator = st.vicinity || 'ไม่ระบุที่อยู่'; // Google จะให้ที่อยู่แบบย่อมาใน vicinity
            ...
            <div style="text-align: center; font-family: 'Segoe UI', sans-serif;">
                ...
                <span style="color: #6b7280; font-size: 12px;">พิกัด: ${stOperator}</span><br>
                <span style="color: #10b981; font-size: 12px;">บนเส้นทางช่วง กม. ที่ ${stationFound.atKm}</span><br>
                ${st.rating ? `<span style="color: #f59e0b; font-size: 12px;">⭐ ${st.rating} ดาว</span>` : ''}
            </div>
        `);
                    ...
                    timelineWaypoints.push({
                        ...
                        desc: `ที่อยู่: ${stOperator} (ช่วง กม. ที่ ${stationFound.atKm})`,
                        chargeTimeMins: 45,
                        targetBattery: 90
                    });
...
function fetchStationsInView() {
    ...
    // 💡 เปลี่ยนมายิง API ผ่าน Backend แทน
    const ocmUrl = `/api/stations/bounds?n=${n}&w=${w}&s=${s}&e=${e}`;
